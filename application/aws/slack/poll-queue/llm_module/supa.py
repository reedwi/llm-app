...
def create_bucket_object(supabase, bucket_name: str, path: str, file_contents, file_content_type: str):
    file_return = supabase.storage.get_bucket(id=bucket_name).upload(path=path, file=file_contents, file_options={'content-type': file_content_type})
    return file_return
...
def empty_bucket_folder(supabase, bucket_id: str, folder: str):
    try:
        bucket = supabase.storage.get_bucket(id=bucket_id)
        files = bucket.list(folder)
        for file in files:
            bucket.remove(f"{folder}/{file['name']}")
    except:
