...
def lambda_handler(event, context):
    logger.info('[MNDY_UPDATE_GROUP_NAME]: ENTER')
    auth_token = event['headers']['authorization']
    event['body'] = json.loads(event['body'])
    item_id = event['body']['payload']['inputFields']['itemId']
    board_id = event['body']['payload']['inputFields']['boardId']

    decoded_header = jwt.decode(jwt=auth_token, key=os.getenv('MONDAY_SIGNING_SECRET'), algorithms=['HS256'], audience="https://i7wjyespljxs6jsbxverchcloq0vpybw.lambda-url.us-east-1.on.aws/")
    logger.info(f'[MNDY_UPDATE_GROUP_NAME]: START : {decoded_header["accountId"]}')
    account = supa.get_account(supabase=supabase, monday_account_id=decoded_header['accountId'])
    if not account: 
        logger.error(f'[MNDY_UPDATE_GROUP_NAME]: ERROR : No account found for account_id: {decoded_header["accountId"]}')
    ...
    if not chatbot_row:
        logger.error(f'[MNDY_UPDATE_GROUP_NAME]: ERROR : Could not find corresponding item in database for item_id: {item_id} : {decoded_header["accountId"]}')
        return return_success()
    ...
    group_id = mndy.update_group_name(access_token=decoded_header['shortLivedToken'], board_id=chatbot_row['documents_board_id'], group_id=chatbot_row['group_internal_name'], new_group_title=item_name)
    logger.info(f'[MNDY_UPDATE_GROUP_NAME]: Updated Monday Group : {decoded_header["accountId"]}')
    supa.update_chatbot_row(supabase=supabase, row_id=chatbot_row['id'], name=item_name)
    logger.info(f'[MNDY_UPDATE_GROUP_NAME]: Updated Supabase Chatbot name : {decoded_header["accountId"]}')
    logger.info(f'[MNDY_UPDATE_GROUP_NAME]: END : {decoded_header["accountId"]}')
    return return_success()
