...
def send_document_error(access_token, board_id, item_id, message):
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json"
    }
    ...
    query = f"""
        mutation {{
            change_multiple_column_values (item_id: {item_id}, board_id: {board_id} column_values: {json_data}) {{
                id
            }}
        }}
    """
